...
contract OilXNftMetadata is WithdrawContractsOwnable {
  constructor() WithdrawContractsOwnable(msg.sender) {}

  string private _mediaUrl = "https://oilxcoin.io/nft-media/";

  function getTokenTypeString(OilXNftTokenType tokenType)
    private
    pure
    returns (string memory strType)
  {
    string memory strTokenType;
    if (tokenType == OilXNftTokenType.DIAMOND) strTokenType = "DIAMOND";
    else if (tokenType == OilXNftTokenType.PLATINUM) strTokenType = "PLATINUM";
    else if (tokenType == OilXNftTokenType.GOLD) strTokenType = "GOLD";
    else if (tokenType == OilXNftTokenType.SILVER) strTokenType = "SILVER";
    else if (tokenType == OilXNftTokenType.BLACK) strTokenType = "BLACK";
    else revert("Invalid NFT type");

    return strType = strTokenType;
  }

  function getTokenTypeNumberString(OilXNftTokenType tokenType)
    private
    pure
    returns (string memory strType)
  {
    string memory strTokenType;
    if (tokenType == OilXNftTokenType.DIAMOND) strTokenType = "0";
    else if (tokenType == OilXNftTokenType.PLATINUM) strTokenType = "1";
    else if (tokenType == OilXNftTokenType.GOLD) strTokenType = "2";
    else if (tokenType == OilXNftTokenType.SILVER) strTokenType = "3";
    else if (tokenType == OilXNftTokenType.BLACK) strTokenType = "4";
    else revert("Invalid NFT type");
    ...
    returns (string memory strAttribute)
  {
    // string memory strAttribute;
    strAttribute = string(
      abi.encodePacked("{", '"trait_type": "', traitType, '",', '"value": "', value, '"', "}")
    );
    return strAttribute;
  }
    ...
    returns (string memory strAttribute)
  {
    // string memory strAttribute;
    strAttribute =
      string(abi.encodePacked("{", '"trait_type": "', traitType, '",', '"value": ', value, "}"));
    return strAttribute;
  }
  ...
  function generateTexts(
    ...
    title = string(
      abi.encodePacked(
        "OilXCoin ", getTokenTypeString(tokenType), " NFT (", strOilXamountText, " OXC Entitlement)"
      )
    );

    description = string(
      abi.encodePacked(
        "This NFT entitles its holder to a one-time claim of its corresponding number of OilXCoin tokens. Once the respective vesting",
        " period has expired, these tokens can be traded freely. The terms of the NFT can be found under https://oilxcoin.io/en/nft/term-sheet.\\n\\n",
        "Contract Address: ",
        contractAddr,
    ...
    description = string(
      abi.encodePacked(
        ...
        "\\nEntitlement: ",
        strOilXamountText,
        " OXC | Entitlement claimed: ",
        strOilXamountClaimedText,
        " OXC | Royalties claimed: ",
        strOilfeeText,
        " OXC"
  ...
  function getMetaAttributes(
    ...
    strAttributes = string(
      abi.encodePacked(
        ...
        ",",
        addMetadataAttributeNumber("OilXCoin amount", strOilXamountText),
        ",",
  ...
  function getTokenURI(address addressNft, uint256 tokenId) public view returns (bytes memory) {
    ...
    bytes memory data =
      getDataBytes(strTitle, strDescription, imageUrl, animationUrl, jsonAttributes);
